...
//        nh_.setParam("/stereo/left/left_camera/auto_exposure","0");

 // Subscribe to input video feed and publish output video feed

        image_sub_left = it_.subscribe("/stereo/left/image_raw", 1, &change_exp_n_save::imageCb, this);
        image_pub_left = it_.advertise("/stereo/left/image_rect_small", 1);
        cv::namedWindow(OPENCV_WINDOW_LEFT);


      //  image_sub_right = it_.subscribe("/stereo/right/image_raw", 1, &resize_n_save::imageCb_right, this);
      //  image_pub_right = it_.advertise("/stereo/right/image_rect_small", 1);
 ...
 void imageCb(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        if (i>10)
        {
            ...
            time_t now;
            time(&now);
            cv::imwrite(std::string("/home/aero/Pictures/SRR_Training_Dataset/hook_wha/tmp/")+ctime(&now)+".jpg",cv_ptr->image);
...
{
    ros::init(argc, argv, "change_exp_n_save");
    change_exp_n_save ic;
    ros::spin();
    return 0;
}
